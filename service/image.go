...
func (svc *ImageService) fetchMissingImage(media *nft_proxy.Media, cacheName string) error {
	...
	var err error
	var data []byte
	if strings.Contains(media.ImageUri, constants.BASE64_PREFIX) {
		base64String := media.ImageUri
		// Remove the data:image/jpeg;base64, prefix if present
		if v := strings.Index(base64String, constants.BASE64_PREFIX); v > -1 {
			base64String = base64String[v+len(constants.BASE64_PREFIX):]
		}
